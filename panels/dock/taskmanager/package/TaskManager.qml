...
ContainmentItem {
    ...
    // TextMetrics 用于精确计算文本宽度
    TextMetrics {
        id: textMetrics
        font.pixelSize: 20  // 会被动态设置
        font.family: "SourceHanSansSC"
    }

    // 使用 TextMetrics 精确计算文本宽度
    ...
    function calculateItemWidth(title, maxChars, iconSize, textSize) {
        ...
        if (titleLength <= maxChars) {
            ...
            displayLen = Math.min(6, maxChars)
        }
    ...
    function calculateTotalWidth(charLimits, iconSize, textSize) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            totalIconWidth += calculateItemWidth(item.model.title, maxChars, iconSize, textSize)
        }
        ...
        return totalIconWidth + totalSpacing
    }
            ...
            delegate: Item {
                ...
                implicitWidth: useColumnLayout ? taskmanager.implicitWidth :
                                (taskmanager.Applet.windowSplit && (Panel.position == Dock.Bottom || Panel.position == Dock.Top)
                                     ? (appLoader.item ? appLoader.item.actualWidth : visualModel.cellWidth)
